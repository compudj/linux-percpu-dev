...
static inline pid_t cpu_mutex_gettid(void)
{
...
struct deref_test_data_entry {
	int padding;
	intptr_t v;
};

struct deref_test_data_p {
	struct deref_test_data_entry *p;
} __attribute__((aligned(128)));

struct deref_test_data {
	struct deref_test_data_p c[CPU_SETSIZE];
};

struct deref_thread_test_data {
	struct deref_test_data *data;
	long long reps;
	int reg;
};
...
#ifndef BENCHMARK
		if (i != 0 && (reps < 10 || !(i % (reps / 10))))
			printf_verbose("tid %d: count %lld\n", (int) cpu_mutex_gettid(), i);
#endif
	}
	printf_verbose("tid %d: number of rseq abort: %d, signals delivered: %u\n",
		       (int) cpu_mutex_gettid(), nr_abort, signals_delivered);
	if (!opt_disable_rseq && thread_data->reg &&
...
void *test_percpu_deref_thread(void *arg)
{
	struct deref_thread_test_data *thread_data = arg;
	struct deref_test_data *data = thread_data->data;
	long long i, reps;
	...
	for (i = 0; i < reps; i++) {
		...
		cpu = rseq_cpu_start();
		ret = percpu_deref_loadoffp((intptr_t *) &data->c[cpu].p,
				offsetof(struct deref_test_data_entry, v),
				&load, cpu);
		if (rseq_unlikely(ret)) {
			perror("cpu_mutex");
			abort();
...
void test_percpu_deref(void)
{
	const int num_threads = opt_threads;
	int i, ret;
	long long iter, iter_reps;
	pthread_t test_threads[num_threads];
	struct deref_test_data data;
	struct deref_thread_test_data thread_data[num_threads];
	struct deref_test_data_entry percpu_v[CPU_SETSIZE][2];
	int current_index = 0;
	int nr_online_cpus;

	nr_online_cpus = get_nprocs();
	assert(nr_online_cpus >= 0);
	memset(&data, 0, sizeof(data));
	memset(percpu_v, 0, sizeof(percpu_v));

	for (i = 0; i < CPU_SETSIZE; i++) {
		percpu_v[i][0].padding = 1;
		percpu_v[i][1].padding = 1;
		data.c[i].p = &percpu_v[i][0];
	}
	...
	iter_reps = opt_reps / nr_online_cpus;
	for (iter = 0; iter < iter_reps; iter++) {
		current_index = 1 - current_index;

		for (i = 0; i < nr_online_cpus; i++) {
			percpu_v[i][current_index].v = 0;
			rseq_smp_wmb();	/* order stores before assign pointer. */
			RSEQ_WRITE_ONCE(data.c[i].p, &percpu_v[i][current_index]);
			/* rseq fence */
			percpu_fence(i);
			percpu_v[i][1 - current_index].v = 1;
		}
	}
...
static void show_usage(int argc, char **argv)
{
	...
	printf("	[-T test] Choose test: (s)pinlock, (l)ist, (b)uffer, (m)emcpy, (i)ncrement, (d)ereference\n");
...
int main(int argc, char **argv)
{
	...
	case 'i':
		...
		break;
	case 'd':
		printf_verbose("pointer dereference\n");
		test_percpu_deref();
		break;
	}
	if (!opt_disable_rseq && rseq_unregister_current_thread())
